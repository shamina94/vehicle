...
def customer_signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if userForm.is_valid() and customerForm.is_valid():
            ...
            email_template_name = 'vehicle/email_verfication.html'
...
def mechanic_signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if userForm.is_valid() and mechanicForm.is_valid():
            ...
            message = render_to_string('vehicle/verify_email.html', {
                'user': user,
...
def mechanic_activate_view(request, uidb64, token):
    ...
    if user and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        return render(request, 'vehicle/activation_success.html', {})
    else:
        return render(request, 'vehicle/activation_failed.html', {})
